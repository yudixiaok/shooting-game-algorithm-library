...
void dsSound::MoveVolume( LONG Volume )
{
	LONG oV	;
	if( !m_Buffer )
		return ;
...
bool WavSound::CreatSound(const char* filename, const wchar_t* index)
{
	...
	newsound->MoveVolume(m_soundloud);
	m_SoundMap.insert(std::make_pair(index, newsound));
	return true;
}

bool WavSound::CreatSound(const char* filename, const wchar_t* index, int dupnum)
{
	if(m_SoundMap.find(index) != m_SoundMap.end())
		return false;
	...
	newdupsound->MoveVolume(m_soundloud);
	m_DupSoundMap.insert(std::make_pair(index, newdupsound));
	return true;
}

bool WavSound::CreatSound(const wchar_t* filename, const wchar_t* index)
{
	if(m_SoundMap.find(index) != m_SoundMap.end())
		return false;
	dsSound *newsound = new dsSound();
...
bool WavSound::CreatSound(const wchar_t* filename, const wchar_t* index, int dupnum)
{
	if(m_SoundMap.find(index) != m_SoundMap.end())
		return false;
	dsDuplicate *newdupsound = new dsDuplicate();
...
bool WavSound::Play(const wchar_t* index)
{
...
bool WavSound::Play(const wchar_t*index, int index2)
{
	//如果能成功執行play就傳回true,其餘皆傳false
	if (!isStart)
		return false;
...
bool WavSound::Stop(const wchar_t* index)
{
...
bool WavSound::Stop(const wchar_t* index, int index2)
{
	//如果能成功執行stop就傳回true,其餘皆傳false
	if (!isStart)
		return false;
...
void WavSound::Release(const wchar_t* index )
{
	m_SoundMapPos = m_SoundMap.find(index);
	if (m_SoundMapPos != m_SoundMap.end())
	{
...
void WavSound::SetVolume( wchar_t* index, LONG Volume )
{	
	m_SoundMapPos = m_SoundMap.find(index);
	if (m_SoundMapPos != m_SoundMap.end())
	{
		m_SoundMapPos->second->MoveVolume(Volume);
		m_soundloud = Volume;
		return;
	}
	m_DupSoundMapPos = m_DupSoundMap.find(index);
	if (m_DupSoundMapPos != m_DupSoundMap.end())
	{
		m_DupSoundMapPos->second->MoveVolume(Volume);
		m_soundloud = Volume;
		return;
	}
	m_soundloud = -1500;
...
LONG WavSound::GetVolume( wchar_t* index )
{	
	m_SoundMapPos = m_SoundMap.find(index);
	if (m_SoundMapPos != m_SoundMap.end())
	{
...
bool WavSound::isExist( const wchar_t* index )
{
	m_SoundMapPos = m_SoundMap.find(index);
	if (m_SoundMapPos != m_SoundMap.end())
		return true;
